...
// LefthandSide L ::= I | L . I | this | super
...
object Babyofieldsuper extends JavaTokenParsers with OpTree {
  ...
  def main(args: Array[String]): Unit = {
    try {
      val source = args(0)
//      val source = "x = 2;y=new{a=x; b=a};z =new{ q=x;a =y.a}"
//      val source = "x = 2;y=new{a =x; b =a};z =new{ q=x;t=this.q}"
//      val source = "x = 2;y=new{a =x; b =a};z =new{ q=x;t=super.x}"
//      val source = "x = 2;y=new{a =x; b =a};z =new{ q=x;t=y.x}"
//      val source = "x = 2;y=new{a =x; b =a};z =new{ q=x;t=super.y.x}"
//      val source = "x = 2;y=new{a =x; b =a};z =new{ q=x;t=super.y.a}"        
...
// input : x = 2;y=new{a =x; b =a};z =new{ q=x;t=super.y.a}
// optree : List(Assign(Id(x),Num(2)), Assign(Id(y),New(List((a,Deref(Id(x))), (b,Deref(Id(a)))))), Assign(Id(z),New(List((q,Deref(Id(x))), (t,Deref(Dot(Dot(Id(super),Id(y)),Id(a))))))))
// namespace : List(Handle(0))
// heap : Map(Handle(0) -> Map(x -> Value(2), y -> Handle(1), z -> Handle(2)), Handle(1) -> Map(a -> Value(2), b -> Value(2)), Handle(2) -> Map(q -> Value(2), t -> Value(2)))

// input : x = 2;y=new{a =x; b =a};z =new{ q=x;t=super.y.x}
// optree : List(Assign(Id(x),Num(2)), Assign(Id(y),New(List((a,Deref(Id(x))), (b,Deref(Id(a)))))), Assign(Id(z),New(List((q,Deref(Id(x))), (t,Deref(Dot(Dot(Id(super),Id(y)),Id(x))))))))
// java.lang.Exception: x is not a member field y

// input : x = 2;y=new{a =x; b =a};z =new{ q=x;t=y.x}
// optree : List(Assign(Id(x),Num(2)), Assign(Id(y),New(List((a,Deref(Id(x))), (b,Deref(Id(a)))))), Assign(Id(z),New(List((q,Deref(Id(x))), (t,Deref(Dot(Id(y),Id(x))))))))
// java.lang.Exception: x is not a member field y

// input : x = 2;y=new{a =x; b =a};z =new{ q=x;t=super.x}
// optree : List(Assign(Id(x),Num(2)), Assign(Id(y),New(List((a,Deref(Id(x))), (b,Deref(Id(a)))))), Assign(Id(z),New(List((q,Deref(Id(x))), (t,Deref(Dot(Id(super),Id(x))))))))
// namespace : List(Handle(0))
// heap : Map(Handle(0) -> Map(x -> Value(2), y -> Handle(1), z -> Handle(2)), Handle(1) -> Map(a -> Value(2), b -> Value(2)), Handle(2) -> Map(q -> Value(2), t -> Value(2)))

// input : x = 2;y=new{a =x; b =a};z =new{ q=x;t=this.q}
// optree : List(Assign(Id(x),Num(2)), Assign(Id(y),New(List((a,Deref(Id(x))), (b,Deref(Id(a)))))), Assign(Id(z),New(List((q,Deref(Id(x))), (t,Deref(Dot(Id(this),Id(q))))))))
// namespace : List(Handle(0))
// heap : Map(Handle(0) -> Map(x -> Value(2), y -> Handle(1), z -> Handle(2)), Handle(1) -> Map(a -> Value(2), b -> Value(2)), Handle(2) -> Map(q -> Value(2), t -> Value(2)))
