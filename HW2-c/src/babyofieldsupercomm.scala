...
trait OpTree {
  ...
  case class New(xs: List[Ctree]) extends Etree
...
object Babyofieldsupercomm extends JavaTokenParsers with OpTree {
  ...
  def interpretETREE(e: Etree): Rval = e match {
    ...
    case New(cl) => {
      ...
      interpretCLIST(cl) // cl 타입에 맞게 계산해줍니다.(인터프리터가 어떻게 작동하는지 잘생각해보세요)
      ns = ns.tail // ns앞에 추가한 핸들값을 빼준 결과를 ns에 저장합니다.
      newhandle// 새로 만든 핸들변수를 내어줍니다.
    } 
  }
  
  // this 의 의미는 Namespace stack 에서 top을 가져오는 것입니다.
  // super 의 의미는 Namespace stack 에서 top을 제거하고 다음 top을 가져오는 것입니다.
  def interpretLTREE(left: Ltree): (Handle,String) = {
    left match {
      case Id(x) => (ns.head,x)
      case Dot(ls,Id(x)) => {
        val (han,lx) = interpretLTREE(ls)
        val handle = lx match {
          // 아래 세가지 case에 맞게 코드를 작성하세요
          case "this" => ns.head
          case "super" => ns.tail.head
          case lvar => { 
            val (h,lx1) = find(List(han)++ns,lx)
            lookup(h,lx1) match {
        ...
        else throw new Exception(x+" is not a member field "+lx)
      }
      case _ => throw new Exception("This case will never be selected.")
    }
  } 
  ...
  def main(args: Array[String]): Unit = {
    try {
      val source = args(0)
//      val source = "x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= 4}; print c };q = new { t = y.f.a }"
//      val source = "x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= 4}; print c }"
//      val source = "x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= super.x}; print c };q = new { t = y.f.a }"
//      val source = "x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= 10}; print c };q = new { t = super.y.a }"
//      val source = "x = 7;y = new { if x : a=x else b = 0 end; c = this.a;f=new{ a= 10}; print c };q = new { t = super.y.b }"
...
// input : x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= 4}; print c };q = new { t = y.f.a }
// optree : List(Assign(Id(x),Num(7)), Assign(Id(y),New(List(Cond(Deref(Id(x)),List(Assign(Id(a),Deref(Id(x)))),List(Assign(Id(b),Num(0)))), Assign(Id(c),Deref(Id(x))), Assign(Id(f),New(List(Assign(Id(a),Num(4))))), Print(Id(c))))), Assign(Id(q),New(List(Assign(Id(t),Deref(Dot(Dot(Id(y),Id(f)),Id(a))))))))
// Value(7)
// namespace : List(Handle(0))
// heap : Map(Handle(0) -> Map(x -> Value(7), y -> Handle(1), q -> Handle(3)), Handle(1) -> Map(a -> Value(7), c -> Value(7), f -> Handle(2)), Handle(2) -> Map(a -> Value(4)), Handle(3) -> Map(t -> Value(4)))

// input : x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= 4}; print c }
// optree : List(Assign(Id(x),Num(7)), Assign(Id(y),New(List(Cond(Deref(Id(x)),List(Assign(Id(a),Deref(Id(x)))),List(Assign(Id(b),Num(0)))), Assign(Id(c),Deref(Id(x))), Assign(Id(f),New(List(Assign(Id(a),Num(4))))), Print(Id(c))))))
// Value(7)
// namespace : List(Handle(0))
// heap : Map(Handle(0) -> Map(x -> Value(7), y -> Handle(1)), Handle(1) -> Map(a -> Value(7), c -> Value(7), f -> Handle(2)), Handle(2) -> Map(a -> Value(4)))

// input : x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= super.x}; print c };q = new { t = y.f.a }
// optree : List(Assign(Id(x),Num(7)), Assign(Id(y),New(List(Cond(Deref(Id(x)),List(Assign(Id(a),Deref(Id(x)))),List(Assign(Id(b),Num(0)))), Assign(Id(c),Deref(Id(x))), Assign(Id(f),New(List(Assign(Id(a),Deref(Dot(Id(super),Id(x))))))), Print(Id(c))))), Assign(Id(q),New(List(Assign(Id(t),Deref(Dot(Dot(Id(y),Id(f)),Id(a))))))))
// java.lang.Exception: x is not a member field super

// input : x = 7;y = new { if x : a=x else b = 0 end; c = x;f=new{ a= 10}; print c };q = new { t = super.y.a }
// optree : List(Assign(Id(x),Num(7)), Assign(Id(y),New(List(Cond(Deref(Id(x)),List(Assign(Id(a),Deref(Id(x)))),List(Assign(Id(b),Num(0)))), Assign(Id(c),Deref(Id(x))), Assign(Id(f),New(List(Assign(Id(a),Num(10))))), Print(Id(c))))), Assign(Id(q),New(List(Assign(Id(t),Deref(Dot(Dot(Id(super),Id(y)),Id(a))))))))
// Value(7)
// namespace : List(Handle(0))
// heap : Map(Handle(0) -> Map(x -> Value(7), y -> Handle(1), q -> Handle(3)), Handle(1) -> Map(a -> Value(7), c -> Value(7), f -> Handle(2)), Handle(2) -> Map(a -> Value(10)), Handle(3) -> Map(t -> Value(7)))

// input : x = 7;y = new { if x : a=x else b = 0 end; c = this.a;f=new{ a= 10}; print c };q = new { t = super.y.b }
// optree : List(Assign(Id(x),Num(7)), Assign(Id(y),New(List(Cond(Deref(Id(x)),List(Assign(Id(a),Deref(Id(x)))),List(Assign(Id(b),Num(0)))), Assign(Id(c),Deref(Dot(Id(this),Id(a)))), Assign(Id(f),New(List(Assign(Id(a),Num(10))))), Print(Id(c))))), Assign(Id(q),New(List(Assign(Id(t),Deref(Dot(Dot(Id(super),Id(y)),Id(b))))))))
// Value(7)
// java.lang.Exception: b is not a member field y
